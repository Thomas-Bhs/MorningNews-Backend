...
Flow: 1. User signs in → access token + refresh token 2. Access token expires → client calls /users/refresh-token 3. New access token + refresh token are generated 4. Logout invalidates the refresh token
...
This API follows RESTful conventions and uses standard HTTP status codes:
